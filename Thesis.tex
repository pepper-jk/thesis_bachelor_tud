...
Each packet contains a variety of header fields, which vary in importance.
...
It only matters if the checksum matches the header.
...
Lets take a closer look at the header fields now.
...
\paragraph{Not Used} are header fields that do not change either never or in our scenario.
...
\paragraph{Trivial} fields might be useful, but should not be fed to the NN.
...
This includes checksums, header lengths, urgent pointer, as well as MAC and IP addresses.

\paragraph{Feature Derivations} are fields, which are used as input to the NN,
but are derived in some way before hand.
They are not raw. % TODO: wording
...
The data offset and IHL (??? same as hl above) are used as an indicating boolean for TCP and IP options respectively.

% TODO
The ports ...

\paragraph{Features} are fields that we consider unmodified or raw input.
...
\begin{table}
    ...
    \begin{tabular}{|r*{32}{|c}|}
        ...
        \hline
        64  & \multicolumn{8}{|c|}{\cellcolor{feature} Time To Live} & \multicolumn{8}{|c|}{\cellcolor{feature} Protocol} & \multicolumn{16}{|c|}{\cellcolor{derivation} Header Checksum} \\
        \hline
...
\begin{table}
    ...
    \begin{tabular}{|r*{32}{|c}|} % m{0.2pt}
        ...
        \hline
        64  & \multicolumn{32}{|c|}{\cellcolor{derivation} Acknowledgement Number} \\
...
\begin{table}
    ...
    \begin{tabular}{|r*{32}{|c}|}
        ...
        0   & \multicolumn{16}{|c|}{\cellcolor{derivation} Source Port} & \multicolumn{16}{|c|}{\cellcolor{derivation} Desitnation Port} \\
        \hline
        32  & \multicolumn{16}{|c|}{\cellcolor{trivial} Length} & \multicolumn{16}{|c|}{\cellcolor{derivation} Checksum} \\
        \hline
    \end{tabular}
    \caption{UDP Header}
\end{table}
