...
% FIXME: remove this and directly start with Image Desc... section
...
% maybe the dataset with the least errors is the baseline
% the dataset with more is our real dataset? o.O
...
% FIXME: one capture each might be bad (sound bad) too late now
...
Each negative sample type has a parameter, which decides the degree to which it alters the ground truth sample it extracts from the dataset of positive datapoints.
The degree of method one is defined by the number of errors assigned to every packet.
The degree of method three is defined by the number of packets added to a sequence or removed from the sequence.
While adding packets was done by a total amount, removing packets was done relative to the sequence size, as having a lot of negative samples with sequences of small size might have caused the NN to learn that small sequences are per default negative.
...
This is already solved in the dateset creation.
...
We wont have to do this as our flow data is already in a continuous state after the dateset creation.
...
So when facing datasets with inuniform shaped datapoints, such as ours, one would need to create batches based on shape instead of randomly.
...
To prevent this each seqeunce and the number of features per input shall be the same for each datapoint.
...
Using RNNs will ensure previous data points will be taken into account during learning and in case of LSTMs and GRUs all previous datapoints will be taken into account.
...
The \lstinline{cond-rnn} library \cite{remyPhilipperemyCondRnn2020} written for Keras and Tensorflow and supports simpleRNNs, LSTMs and GRUs as cells for conditional RNNs.
...
As our model performed suboptimal, we first shifted our focus from comparing the different RNN architectures to finding hyperparameters to solve our problems.
But we still aimed to compare the three RNNs.
...
For our initial test runs of our model we used only around 5\% and 10\% of the dataset as not to exceed the resources, specifically memory, of the systems we used.
...
Even though we had longer compute times, this allowed us to run experiments with 25\% of our dataset and use more than 96 units within an RNN layer.
...
As the accuracy was most often pending around 57\% over multiple runs and the same ratio of positive to negative datapoints in our dataset (test2 \ref{tab:datasets}), we assumed its true accuracy is at best 50\%, so not better as a coin toss.

Our advanced model for further experiments therefore consists of two masking layers, which provide the RNN layer with the mask for UDP and packet padding,
up to four condRNN layers with up to 128 units,
...
This was manually implemented as a \lstinline{keras.Model}, since the condRNN layer from the \lstinline{cond-rnn} library is incompatible with \lstinline{keras.Sequential} at this point in time with its two inputs.
% UNITS
...
Throughout our experiments we used 70\%, 10\%, and 20\% of our partial datasets for training, validation and testing respectively, as it is the most common segmentation.
For epochs we started with 5 to 20 for initial testing of hyperparameter combinations and went up to 100 or as far as 1000 once a combination looked promising.
...
% EPOCHS
Usually the accuracy grows over the first three to 20 epochs, depending on the experiment and its hyperparameters, and then hovers around 57\%.
Sometimes grew very slowly, but even 1000 epochs weren't enough to reach a decent accuracy. % FIXME once final results are in
% LAYERS
However we still only reached an accuracy of 59\% at best with one RNN layer.
...
In our new experiments, since the switch to binary cross-entropy, we saw the same behavior, only this time 2 layers seem to achieve the optimal results.

% LOSS FUNCTION
For a long time we used sparse categorical cross-entropy as a loss function, but this function was ill-suited for our problem.
Once we figured this out we switched to the binary cross-entropy loss function, which is best suited for binary classification problems.
Combined with a sigmoid activation for out output layer this helped us obtain better results.
We also compared our results to the hinge loss function in order to have a steeper learning curve, as it is used in combination with the hyperbolic tangent function on the output of the NN and provides a greater punishment for misclassification with the wrong signage.
We thought this might be beneficial to our cause, but as it turns out it lead to worse results, see figure \ref{fig:hinge}.

% FIXME transition? maybe rewrite
...
We tried a dataset using data from only one of the afore mentioned NIDS dataset captures with no improvements.
Changing the architecture from the conditional RNN to a normal RNN did not change the results too much, just a little worse, so a problem with the \lstinline{cond-rnn} library can be excluded.
...
However we still saw no noticable changes to the accuracy.
...
At least regarding the validation accuracy and loss.
