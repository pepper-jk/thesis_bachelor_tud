...
My personal workstation with an AMD Ryzen 5 3600 6-Core @ 12x 3.6GHz and 32GB of ECC memory and TKs compute server HULK with the Tesla V100S GPU also with 32GB memory.
As our experiments progressed and we required more memory, we had to switch to CPU processing on HULK.
Giving us a share of the 48 threads @ 3.0GHz and 504GB of memory.
...
This helped us obtain better results.
...
Our advanced model for futher experiments therefore consists of two masking layers, which provide the RNN layer with the mask for UDP and packet padding,
up to four condRNN layers with up to 128 units,
a dense layer with ReLU activation and half as many units as a RNN layer,
...
% FIXME: why use 500 than

Training the NN appeared as a problematic task.
The model did not seem to achieve more than 60\% accuracy.
As its accuracy was pending around 57\% over multiple runs and the data used for training had around the same ratio of positive to negative datapoints, we assumed its true accuracy is at best 50\%, so not better as a coin toss.
