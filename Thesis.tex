...
The Explicit Congestion Notification (ECN) header field is a simple example.
...
It is first divided into 3 pools of values, specified by the values last two bits.
...
The five categories Low Effort, Class Selector, Assured Forwarding, Voice Admit, and Expedited Forwarding will again be used as bool features for the NN input.
...
Both of these classifications will be be represented as an integer value.
...
This goes for the total Length, ID, TTL, Protocol, and Window Size.
...
To use our dataset with a NN we need to convert the datapoints into a data structure that suited as NN input.
As our NN works best with continuous data we face the following problems when converting the data:
First the data that is only available as bytes and strings.
...
Second those sequences of binary representations need to be converted into continuous sequences, before being concatenated with the features, which are continuous by design. %eh, from the beginning is what I mean
...
Our data has a general structure of flow data concatenated with a sequence of packets.
As RNNs work with sequential data, in our case only the sequence of packets, we need a more sophisticated solution to take our flow data into account.
There are multiple approaches to this problem.
One is the append the flow data to each packet in the sequence.
But this creates an enormous amount of overhead.
Another approach would be to use a different Layer to learn about the flow data, but as the flow data directly impacts the packet sequence data, separating them would not be a good idea.
The only liable approach is to modify the (initial) state of the RNN with the flow data as proposed by Vinyals et al. and by Karpathy and Fei-Fei.
% TODO: references
