...
since a capture itself is truncated at beginning and end,
so it should not be a problem.
The second issue is, the length of the intervals.
Should they be determined by percentage or by a fixed length.
While the fixed length has better comparability,
the percentage is the less time consuming.

Extracting the packet sequences from a capture is the more time consuming,
but might yield deeper results.
In the sense that some flows might actually be complete
and the variaty of sequence length might yield better results later on.

For either of the above options it is to be decided,
if the sampling will be done sequencially or randomly.
...
We suggest a variaty of patterns to choose the packet sequences taken into account,
when creating the dataset.
With adding a new meta boolean attribute \textbf{truncated},
we also can mark the truncated flows.
This will help the NN to seperate the two categories of packet sequences.

%Detecting truncation:
%- just assume? (since the catpure is split into intervals)
%- or check given the sequence number and "stream number"
